...
public class IntegratedMiningDivision {
        ...
        for (MaterialRegistry materialRegistry : GregTechAPI.materialManager.getRegistries())
        {
            for (Material material : materialRegistry) {
                if (material.hasProperty(PropertyKey.ORE)) {
                    addIntegratedMiningRecipe1A(material, 3)
                            .EUt(480).duration(20)
                            .buildAndRegister();
                    addIntegratedMiningRecipe1B(material, 3)
                            .EUt(480).duration(20)
                            .buildAndRegister();
    ...
    private static SimpleRecipeBuilder addIntegratedMiningRecipe1A(Material material, int output) {
    ...
    private static SimpleRecipeBuilder addIntegratedMiningRecipe1B(Material material, int output) {
        return GTQTcoreRecipeMaps.INTEGRATED_MINING_DIVISION.recipeBuilder()
                .input(OrePrefix.rawOre, material)
                .fluidInputs(Water.getFluid(1000))
                .circuitMeta(1)
                .output(OrePrefix.dust, material, output);
    }
