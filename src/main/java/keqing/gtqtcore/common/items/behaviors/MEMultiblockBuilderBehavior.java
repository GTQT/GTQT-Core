...
public class MEMultiblockBuilderBehavior implements IItemBehaviour, ItemUIFactory {
    ...
    // 在类成员变量区域添加
    private static final Queue<CraftingRequest> craftingQueue = new LinkedList<>();
    ...
    public List<ItemStack> dealWithMultiblock(MultiblockControllerBase multiblock) {
        ...
        for (ItemStack itemStack : original) {
            ...
            // 关键修改：创建原始ItemStack的副本，所有操作基于副本
            ItemStack modifiedStack = itemStack.copy(); // 创建副本
            int count = modifiedStack.getCount();      // 获取副本数量
            modifiedStack.setCount(count * multiple);  // 仅修改副本数量

            boolean out = false;

            // 后续所有操作使用 modifiedStack 代替 itemStack
            if (multiblock.getStackForm().isItemEqual(modifiedStack)) continue; // 比较副本

            if (!setHatch) {
                Block block = Block.getBlockFromItem(modifiedStack.getItem()); // 使用副本
                if (block instanceof BlockMachine) continue;
            }

            ItemStack hatchStack = modifiedStack.copy(); // 基于副本创建hatchStack
            ...
            if (out) continue;
            itemStackList.add(modifiedStack);
        }
    ...
    public ModularUI createUI(PlayerInventoryHolder playerInventoryHolder, EntityPlayer entityPlayer) {
        return ModularUI.builder(GuiTextures.BACKGROUND, 320, 225)
    ...
    private void addDisplayText1(List<ITextComponent> iTextComponents) {
        ...
        iTextComponents.add(new TextComponentTranslation("自动合成：" + autoCraft + " 倍数：" + multiple));
    }
                ...
                if (player.isSneaking()) {
                    if (!multiblock.isStructureFormed()) {
                        ...
                        try {
                            ...
                            IMEMonitor<IAEItemStack> monitor = CraftingHandler.getStorage(player).getInventory(channel);
    ...
    private void extractItemsFromNetwork(IMEMonitor<IAEItemStack> monitor, IAEItemStack itemStack, EntityPlayer player) {
        ...
        //创建需求物品堆
        int requireCount = prototype.getCount();
        ...
        if (requireCount <= networkAvailable) {
            player.sendMessage(new TextComponentTranslation("玩家背包可用 " + playerExistingCount));
            player.sendMessage(new TextComponentTranslation("网络请求 " + requireCount + " 合法,开始请求。"));
        } else {
            long crafting = requireCount - networkAvailable;

            if (autoCraft) {
                player.sendMessage(new TextComponentTranslation("网络缺失 " + crafting + " 正在自动创建下单请求").setStyle(new Style().setColor(TextFormatting.RED)));
                createCraft(prototype, (int) crafting);
                player.sendMessage(new TextComponentTranslation("------------------"));
                return;
            } else {
        ...
        final long requireFromNet = Math.min(networkAvailable, requireCount);
        ...
        player.sendMessage(new TextComponentTranslation("请求完毕！实际向网络请求 " + requireFromNet).setStyle(new Style().setColor(TextFormatting.YELLOW)));
        player.sendMessage(new TextComponentTranslation("------------------"));
    }

    //下单
    public void createCraft(ItemStack item, int count) {
        item.setCount(count);
        craftingQueue.add(new CraftingRequest(item.copy()));
    }
    ...
    public void onUpdate(ItemStack itemStack, Entity entity) {
        if (!entity.world.isRemote && entity instanceof EntityPlayer player) {
            if (!craftingQueue.isEmpty()) {
                CraftingRequest request = craftingQueue.peek();
                if (request == null) return;
                ItemStack item = request.item;
                int slot = CraftingHandler.getSlot(player);
                if (slot == -1) return;
                IGridNode gridNode = CraftingHandler.getGridNode(player, slot);
                if (gridNode == null) return;
                CraftingHandler.openWirelessCraft(player, item, gridNode, slot, false);
                craftingQueue.poll();
            }
        }
    ...
    public void addInformation(ItemStack itemStack, List<String> lines) {
        ...
        lines.add(I18n.format("背包内放置无线合成终端即可绑定至所属AE，对准多方块控制器右键可绑定机器，对准其他方块右键可打开高级调试界面UI"));
        lines.add(I18n.format("对准多方块控制器按住shift可进入高级构建模式，否则只进行普通构建"));
        lines.add(I18n.format("高级构建模式需打开“放置'保险'才可真实放置多方块，默认启用不足下单"));
        ...
        lines.add(I18n.format("需要注意的是部分特殊多方块对能源仓的等级有特定需求，请不要使用错误等级的能源仓"));
    ...
    // 添加一个内部类来封装请求
    private static class CraftingRequest {
        final ItemStack item;

        CraftingRequest(ItemStack item) {
            this.item = item;
        }
    }
