...
public class PressureSingleRecipeLogic extends RecipeLogicEnergy {

    private double recipePressure = GCYSValues.EARTH_PRESSURE;

    public PressureSingleRecipeLogic(MetaTileEntity tileEntity, RecipeMap<?> recipeMap, Supplier<IEnergyContainer> energyContainer) {
    ...
    protected boolean drawPressure(double pressure, boolean simulate) {
        ...
        if (simulate) {
            ...
            return !(newPressure < container.getMinPressure() * 0.75) && !(newPressure > container.getMaxPressure() * 1.25);
        }
